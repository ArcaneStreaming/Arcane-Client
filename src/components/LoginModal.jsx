...
class LoginModal extends Component {
	...
	checkUsername = (e) => {
		...
		if (u.length < 1) { this.setState({ username_errors:['required field'] }); } else if (u.length > 0 && !this.checkSpecialChars(u)) { // and check if not in db
	...
	formHasErrors () {
		...
		if (username_errors.length > 0 || password_errors.length > 0) { return true; } else { return false; }
	}

	formIsEmpty () {
		const { username, password } = this.state;
		if (username.length < 1 || password.length < 1) { return true; } else { return false; }
	}
