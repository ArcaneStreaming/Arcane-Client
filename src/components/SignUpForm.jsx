...
class SignUpForm extends Component {
	...
	checkField = (e) => {
		...
		if (item.length < 1) { this.setState({ [errKey]:['required field'] });} else if (key !== 'email' && item.length > 0 && !this.checkSpecialChars(item)) {
	...
	verifyPassword = (e) => {
		...
		if (p.length < 1) { this.setState({ password_errors:['required field'] }); } else if (p.length > 0 && (!this.checkLength(p, 8) || !this.checkSpecialChars(p))) { // and check if not in db
	...
	formHasErrors () {
		...
		if (email_errors.length > 0 || password_errors.length > 0 || first_name_errors.length > 0 || last_name_errors.length > 0 || username_errors.length > 0 || confirm_errors.length > 0) { return true; } else { return false; }
	}

	formIsEmpty () {
		const { first_name, last_name, email, password, username } = this.state;
		if (!(email && password && first_name && last_name && username)) { return true; } else { return false; }
	}
